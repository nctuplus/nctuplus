...
;(function($, window, document, undefined) {
	...
	Tree.defaults = {
		...
		emptyIcon: 'glyphicon',
		nodeIcon: 'fa fa-star',//'glyphicon glyphicon-stop',
		...
		selectedBackColor: '#BBFFEE',// #A9F5F2 #428bca
...
/* select target node by hash value & expand there */
		activateNode: function (key, value) {
    		this._activateNode(key, value, this.tree);
		},
		
		_activateNodeParents: function(parents) {
   			for (var i = 0; i < parents.length; i++) {
        		if (parents[i]._nodes) {
            		parents[i].nodes = parents[i]._nodes;
            		delete parents[i]._nodes;
        		}
    		}
		},
		_activateNode: function (key, value, nodes, parents) {
    		if (nodes == null)
        		return;
    		for (var i = 0; i < nodes.length; i++) {
       			if (nodes[i][key] == value) {
            		this._activateNodeParents(parents || []);
            		this._setSelectedNode(nodes[i]);
            		if(nodes[i]['type']>0){  //if can expand, expand
            			this._toggleNodes(nodes[i]);
						this._render();
					}
            		return;
        		}
        		var nodeParents = parents ? parents.slice() : [];
        		nodeParents.push(nodes[i]);
        		this._activateNode(key, value, nodes[i].nodes || nodes[i]._nodes, nodeParents);
    		}
		},
/* end */
		_destroy: function() {
		...
		_triggerNodeSelectedEvent: function(node) {
			...
			this.$element.trigger('nodeSelected', [$.extend(true, {}, node)]);  //, this.map_id
		},
				...
				if (node._nodes) {
					treeItem
						.append($(self._template.expandCollapseIcon)
							...
							.css("color", self.options.ECColor )
						);
				}
				else if (node.nodes) {
					treeItem
		...
		_template: {
			...
			expandCollapseIcon: '<span class="expand-collapse" style="/*border:solid;*/"></span>',
